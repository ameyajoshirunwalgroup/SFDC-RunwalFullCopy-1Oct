trigger TrgBrokerageScheme on Brokerage_Scheme__c (before insert,after insert,before update, after update) {
    ...
    if(Trigger.isAfter && Trigger.isUpdate){
        ...
        if (!schemeIdsToRun.isEmpty()) {
            // Step 2: Query all Bookings with your filter
            List<Booking__c> bkList = [
                SELECT Id, Name, Opportunity__c, BrokerIId__c, Agreement_Value_for_brokers__c,
                Project__c, Booking_Date__c, Tower__c, RW_X9_99_Received__c,
                RW_Registration_Done__c, Brokerage_Scheme__c
                FROM Booking__c
            ...
            // Step 3: Collect all CPIds
            Set<Id> cpIds = new Set<Id>();
            for (Booking__c b : bkList) {
                cpIds.add(b.BrokerIId__c);
            }
            
            // Step 4: Loop through updated schemes and call batch for each
            for (Id schemeId : schemeIdsToRun) {
                Set<Id> singleScheme = new Set<Id>{ schemeId };
